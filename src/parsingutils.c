...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/07 15:47:06 by hetya             #+#    #+#             */
/*   Updated: 2022/05/16 16:24:52 by hetya            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_openfile(char *file, int fd, t_bool append, int *lastfd)
{
	...
	if (fd == 0)
	{
		...
		{
			g_return_value = 1;
			perror(ft_strjoin("Finishell: ", file));
			*lastfd = -2;
			return (-2);
		}
	}
	else if (!access(file, F_OK) && access(file, W_OK) == -1)
		return (!!ft_perror(ft_strjoin("Minishell: ", file)) - 1);
	if (fd == 1 && append == 1)
